	...
	try
	{
		...
		while (!in.eof())
		{
			...
			for (; ntaxa > 0; ntaxa--)
			{
				string str;
				if (!(in >> str))
					throw "Cannot read in taxon name";
				if ((ntaxa > 1) && in.eof())
					throw "Unexpected end of file while reading taxon names";
		...
		in.close();
		if (count == 0)
			throw "No set found, you must specify at least 1 set";
	}
	catch (ios::failure)
	{
		outError(ERR_READ_INPUT);
...
void parseArg(int argc, char *argv[], Params &params)
{
	...
	params.num_start_row = 1000000000;
	params.num_add_row = 0;
	...
	for (cnt = 1; cnt < argc; cnt++)
	{
		try
		{
			...
			if (strcmp(argv[cnt], "-pp_orig_tbr") == 0)
			{
				params.pporigtbr = true;
				continue;
			}
			...
			{
				cnt++;
				params.num_start_row = convert_int(argv[cnt]);
				continue;
			}
			if (strcmp(argv[cnt], "-pp_k") == 0)
			{
				cnt++;
				params.num_add_row = convert_int(argv[cnt]);
				continue;
			}
			...
			if (strcmp(argv[cnt], "-vv") == 0 || strcmp(argv[cnt], "-v2") == 0)
			{
				verbose_mode = VB_MAX;
				continue;
			}
			if (strcmp(argv[cnt], "-vvv") == 0 || strcmp(argv[cnt], "-v3") == 0)
			{
			...
			if (strcmp(argv[cnt], "-g") == 0 || strcmp(argv[cnt], "--greedy") == 0)
			{
				params.run_mode = GREEDY;
				continue;
			}
			if (strcmp(argv[cnt], "-pr") == 0 || strcmp(argv[cnt], "--pruning") == 0)
			{
			...
			if (strcmp(argv[cnt], "-dist") == 0 || strcmp(argv[cnt], "-d") == 0)
			{
			...
			else if (strcmp(argv[cnt], "-mprob") == 0)
			{ // compute multinomial distribution probability
				cnt++;
				if (cnt >= argc)
			...
			if (strcmp(argv[cnt], "-aln") == 0 || strcmp(argv[cnt], "-s") == 0)
			{
				cnt++;
				if (cnt >= argc)
			...
			if (strcmp(argv[cnt], "-pdel") == 0 || strcmp(argv[cnt], "-p") == 0)
			{
				cnt++;
				if (cnt >= argc)
			...
			if (strcmp(argv[cnt], "-mod") == 0 || strcmp(argv[cnt], "-m") == 0)
			{
				cnt++;
				if (cnt >= argc)
			...
			if (strcmp(argv[cnt], "-fastsse") == 0 || strcmp(argv[cnt], "-fasttipsse") == 0)
			{
				params.SSE = LK_EIGEN_SSE;
				continue;
			}
			...
			if (strcmp(argv[cnt], "-dmplevel") == 0 || strcmp(argv[cnt], "-dmprank") == 0)
			{
				cnt++;
				if (cnt >= argc)
			...
			if (strcmp(argv[cnt], "-nostore") == 0 || strcmp(argv[cnt], "-memsave") == 0)
			{
				params.store_candidate_trees = false;
				continue;
			}
			...
			if (strcmp(argv[cnt], "-popsize") == 0 || strcmp(argv[cnt], "-numcand") == 0)
			{
				cnt++;
				if (cnt >= argc)
			...
			if (strcmp(argv[cnt], "-fivebran") == 0 || strcmp(argv[cnt], "-nni5") == 0)
			{
				params.nni5 = true;
				params.nni_type = NNI5;
				continue;
			}
			if (strcmp(argv[cnt], "-onebran") == 0 || strcmp(argv[cnt], "-nni1") == 0)
			{
				params.nni_type = NNI1;
				params.nni5 = false;
				continue;
			...
			if (strcmp(argv[cnt], "-lsvar") == 0)
			{
				...
				if (strcmp(argv[cnt], "ft") == 0 || strcmp(argv[cnt], "first_taylor") == 0)
				{
					params.ls_var_type = WLS_FIRST_TAYLOR;
					continue;
				}
				if (strcmp(argv[cnt], "fm") == 0 || strcmp(argv[cnt], "fitch_margoliash") == 0)
				{
					params.ls_var_type = WLS_FITCH_MARGOLIASH;
					continue;
				}
				if (strcmp(argv[cnt], "st") == 0 || strcmp(argv[cnt], "second_taylor") == 0)
				{
					params.ls_var_type = WLS_SECOND_TAYLOR;
					continue;
				}
				if (strcmp(argv[cnt], "p") == 0 || strcmp(argv[cnt], "pauplin") == 0)
				{
			...
			if (strcmp(argv[cnt], "-pb") == 0)
			{ // Enable parsimony branch length estimation
				params.parbran = true;
				continue;
			}
	...
	if (params.sprDist == -1)
	{
		if (params.maximum_parsimony)
		{
			if (!params.spr_parsimony)
				...
				params.sprDist = params.spr_maxtrav;
		}
		else
			params.sprDist = 20;
	}
