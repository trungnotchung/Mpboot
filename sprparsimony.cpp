...
void pllComputePatternParsimonySlow(pllInstance *tr, partitionList *pr, double *ptn_npars, double *cur_npars)
{
  ...
  if (cur_npars)
    *cur_npars = -(iqtree->computeParsimony());
  else
...
void computeUserTreeParsimomy(Params &params)
{
  ...
  if (params.sankoff_cost_file)
  {
    ...
    if (ptree->pllPartitions->partitionData[0]->informativePtnWgt)
    {
      ...
      {
        if (ptree->aln->at(i).frequency != ptnWgt[i])
          diffCount++;
      }
      assert(diffCount == 0);
    }
...
/* Diep begin */
void pllSortedAlignmentRemoveDups(pllAlignmentData *alignmentData, partitionList *pl)
{
  int i, j, k, p;
  char ***sites;
  void **memptr;
...
int _pllAddMoreRow(pllInstance *tr, partitionList *pr)
{
  ...
  evaluateParsimony(tr, pr, tr->start, 1, 0);
