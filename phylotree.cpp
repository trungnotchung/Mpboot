...
void PhyloTree::initializeAllPartialPars(int &index, PhyloNode *node, PhyloNode *dad)
{
    ...
    FOR_NEIGHBOR_IT(node, dad, it)
    initializeAllPartialPars(index, (PhyloNode *)(*it)->node, node);
}
...
int PhyloTree::computeParsimonyBranch(PhyloNeighbor *dad_branch, PhyloNode *dad, int *branch_subst)
{
    ...
    int nptn = aln->size();
    if (!_pattern_pars)
        _pattern_pars = aligned_alloc<BootValTypePars>(nptn + VCSIZE_USHORT);
    ...
    if (aln->num_states == 4 && aln->seq_type == SEQ_DNA)
    {
        ...
        for (ptn = 0; ptn < aln->size(); ptn += 8)
        {
            ...
            if (add_row)
            {
                ...
                save_branch_states_dad[ptn / 8] = states_dad;
            }
        }
    }
...
void PhyloTree::computePatternLikelihood(double *ptn_lh, double *cur_logl, double *ptn_lh_cat)
{
    ...
    if (ptn_lh_cat)
    {
        ...
        else
        {
            ...
            {
            case 4:
                computeLikelihoodBranchEigen<4>(current_it, (PhyloNode *)current_it_back->node);
                break;
            case 20:
                computeLikelihoodBranchEigen<20>(current_it, (PhyloNode *)current_it_back->node);
                break;
            case 2:
                computeLikelihoodBranchEigen<2>(current_it, (PhyloNode *)current_it_back->node);
                break;
            default:
                outError("Option unsupported yet for this sequence type. Contact author if you really need it.");
                break;
            }
        }
    }
        ...
        else
        {
            for (i = 0; i < nptn; i++)
            {
                ptn_lh[i] = _pattern_lh[i] + (max(UBYTE(0), current_it->scale_num[i]) +
                                              max(UBYTE(0), current_it_back->scale_num[i])) *
...
double PhyloTree::optimizeOneBranchLS(PhyloNode *node1, PhyloNode *node2)
{
    ...
    { // Both node are internal node
        ...
        if (params->ls_var_type == OLS)
        {
            ...
            lsBranch = 0.5 * (gamma * (distAC / (A * C) + distBD / (B * D)) + (1 - gamma) * (distBC / (B * C) + distAD / (A * D)) - distAB / (A * B) - distCD / (C * D));
        }
        else
        {
            // weighted least square
            ...
            lsBranch =
                ((distAC / weightAC + distBD / weightBD) * (weightAD + weightBC) / (weightAD * weightBC) +
                 (distAD / weightAD + distBC / weightBC) * (weightAC + weightBD) / (weightAC * weightBD)) /
                    K -
...
double PhyloTree::approxOneBranch(PhyloNode *node, PhyloNode *dad, double b0)
{
    ...
    b1 = b0 - std;
    if (b1 <= 0)
        b1 = beps; /* only happens for b<=1 with small seq. len. */
    b2 = b0 + std;
...
NNIMove PhyloTree::getBestNNIForBran(PhyloNode *node1, PhyloNode *node2, NNIMove *nniMoves)
{
    ...
    if (nniMoves[0].node1)
    {
        ...
        {
            // sanity check
            if (!node1->findNeighbor((*nniMoves[cnt].node1Nei_it)->node))
                outError(__func__);
            if (!node2->findNeighbor((*nniMoves[cnt].node2Nei_it)->node))
                outError(__func__);
        }
    }
    else
    ...
    for (id = IT_NUM - 1; id >= 0; id--)
    {
        ...
        if (params->maximum_parsimony)
            delete[] ((PhyloNeighbor *)(*saved_it[id]))->partial_pars;
        if (*saved_it[id] == current_it)
            current_it = (PhyloNeighbor *)saved_nei[id];
        if (*saved_it[id] == current_it_back)
            current_it_back = (PhyloNeighbor *)saved_nei[id];
    ...
    do
    {
        ...
        for (tax_it = taxa.begin(); tax_it != taxa.end(); tax_it++)
            if (linked_taxid[(*tax_it)->id] < 0)
            {
                ...
                if (!isSupportedNode((PhyloNode *)near_nei->node, min_support) && dist_mat[taxon->id * ntaxa + adj_taxon->id] > 2e-6)
                    continue;
...
bool PhyloTree::checkEqualScalingFactor(double &sum_scaling, PhyloNode *node, PhyloNode *dad)
{
    ...
    if (dad)
    {
        double scaling = ((PhyloNeighbor *)node->findNeighbor(dad))->lh_scale_factor + ((PhyloNeighbor *)dad->findNeighbor(node))->lh_scale_factor;
        if (sum_scaling > 0)
            sum_scaling = scaling;
    ...
    FOR_NEIGHBOR_IT(node, dad, it)
    if (!checkEqualScalingFactor(sum_scaling, (PhyloNode *)(*it)->node, node))
        return false;
            ...
            for (ptn = 0; ptn < aln->size(); ptn += 8)
            {
                ...
                for (int i = 0; i < maxi; i++)
                {
                    ...
                    else
                    {
                        ...
                        left_branch_mutations.push_back(make_pair(p, c1));
                    }
                    for (int j = 0; j < 4; ++j)
                    {
                        if (j != c1 && (1 & (state_left >> j)))
                        {
                            left[ptn / 8] ^= (1 << (i * 4 + j));
                        }
                    }
                    ...
                    else
                    {
                        ...
                        right_branch_mutations.push_back(make_pair(p, c2));
                    }
                    for (int j = 0; j < 4; ++j)
                    {
                        if (j != c2 && (1 & (state_right >> j)))
                        {
                            right[ptn / 8] ^= (1 << (i * 4 + j));
                        }
                    }
...
void PhyloTree::initMutation(vector<int> &permCol, vector<int> &compressedPermCol)
{
    ...
    {
        char nuc = ((save_branch_states_dad[ptn] >> (i * 4)) & 15);
        char ref = aln->reference_nuc[permCol[i]];
        if ((nuc & ref) == 0)
        {
            char c1 = 0;
            for (c1 = 0; c1 < 4; ++c1)
...
void PhyloTree::depth_first_search(PhyloNode *node, PhyloNode *dad)
{
    ...
    FOR_NEIGHBOR_IT(node, dad, it)
    {
        if ((*it)->node->name != ROOT_NAME)
        {
            ...
            childNode->dad = node;
            depth_first_search(childNode, node);
        }
    }
    ...
    FOR_NEIGHBOR_IT(node, dad, it)
    {
        if ((*it)->node->name != ROOT_NAME)
        {
            ((PhyloNeighbor *)(*it))->distance = INFINITY;
            ((PhyloNeighbor *)(*it))->unmark_can_do_spr();
            PhyloNeighbor *it_back = (PhyloNeighbor *)(*it)->node->findNeighbor(node);
            it_back->distance = INFINITY;
            it_back->unmark_can_do_spr();
            if (!it_back->check_can_move())
            {
                check = false;
            }
        }
...
void PhyloTree::depth_first_search()
{
    assert(root->isLeaf());
    PhyloNeighbor *nei = ((PhyloNeighbor *)root->neighbors[0]);
    PhyloNode *node = (PhyloNode *)nei->node;
    node->dad = (PhyloNode *)root;
    nei->distance = INFINITY;
    PhyloNeighbor *nei_back = (PhyloNeighbor *)node->findNeighbor(root);
    nei_back->distance = INFINITY;
...
void PhyloTree::calculatePlacementMutation(CandidateNode &input, bool compute_parsimony_scores, bool compute_vecs)
{
    ...
    {
        visited_missing_sample_mutations[m.compressed_position] = timerRegular;
    ...
    if (!(input.node == root))
    {
        for (auto m1 : input.node_branch->mutations)
        {
            ...
            if (visited_missing_sample_mutations[m1.compressed_position] == timerRegular)
            {
    ...
    {
        visited_ancestral_mutations[m.compressed_position] = timerRegular;
...

    {
        ...
        while (n->dad != root)
        {
            ...
            {
                if (!m.is_masked() && visited_ancestral_mutations[m.compressed_position] != timerRegular)
                {
                    ancestral_mutations.emplace_back(m);
                    anc_positions.emplace_back(m.compressed_position);
                    visited_ancestral_mutations[m.compressed_position] = timerRegular;
    ...
    for (auto m1 : (*input.missing_sample_mutations))
    {
        ...
        if (visited_ancestral_mutations[m1.compressed_position] == timerRegular)
        {
...
void PhyloTree::initDataCalculatePlacementMutation(CandidateNode &inp)
{
    ...
    {
        visited_missing_sample_mutations[m.compressed_position] = timerOptimized;
        cur_missing_sample_mutations[m.compressed_position] = m;
    }
}
...
void PhyloTree::optimizedCalculatePlacementMutation(CandidateNode &input, int set_difference, bool firstNode)
{
    ...
    if (!(input.node == root))
    {
        for (auto m1 : input.node_branch->mutations)
        {
            ...
            bool found = false;
            bool found_pos = false;
            if (visited_missing_sample_mutations[m1.compressed_position] == timerOptimized)
            {
                auto m2 = cur_missing_sample_mutations[m1.compressed_position];
                if (m1.position == m2.position)
                {
    ...
    {
        {
            ...
            while (n->dad != root)
            {
                ...
                for (auto m : node_branch->mutations)
                {
                    if (!m.is_masked() && visited_ancestral_mutations[m.compressed_position] != timerOptimized)
                    {
                        ancestral_mutations.emplace_back(m);
                        anc_positions.emplace_back(m.compressed_position);
        ...
        for (auto m1 : (*input.missing_sample_mutations))
        {
            ...
            if (visited_ancestral_mutations[m1.compressed_position] == timerOptimized)
            {
                auto m2 = cur_ancestral_mutations[m1.compressed_position];
                if (!m2.is_masked())
                {
    ...
    for (auto m : diff_mutations)
    {
        ...
        if (visited_missing_sample_mutations[m.compressed_position] == timerOptimized)
        {
...
void PhyloTree::addNewSample(PhyloNode *best_node, PhyloNeighbor *best_node_branch, std::vector<Mutation> node_excess_mutations, int index, std::string sample_name)
{
    ...
    {
        visited_ancestral_mutations[m1.compressed_position] = timerRegular;
        cur_ancestral_mutations[m1.compressed_position] = m1;
    }
    for (auto m2 : node_excess_mutations)
    {
        visited_excess_mutations[m2.compressed_position] = timerRegular;
        cur_excess_mutations[m2.compressed_position] = m2;
    }
    for (auto m1 : curr_l1_mut)
    {
        bool found = false;
        if (!m1.is_masked())
        {
            if (visited_excess_mutations[m1.compressed_position] == timerRegular)
            {
                auto m2 = cur_excess_mutations[m1.compressed_position];
                if (m1.position == m2.position)
                {
...
string PhyloTree::checkPartialMutation(vector<int> &pos, PhyloNeighbor *dad_branch, PhyloNode *dad)
{
    PhyloNode *node = (PhyloNode *)dad_branch->node;
    int ptn;
    int nstates = aln->num_states;
    ...
    if (nstates == 4 && aln->seq_type == SEQ_DNA && (node->isLeaf() || node->degree() == 3))
    {
        ...
        if (node->isLeaf() && dad)
        {
            PhyloNeighbor *node_branch = (PhyloNeighbor *)node->findNeighbor(dad);
            string s = "";
            if (node->id >= aln->getNSeq())
                cout << node->id << " " << aln->getNSeq() << '\n';
            assert(node->id < aln->getNSeq());
            for (int i = 0; i < (int)aln->getNSite(); ++i)
            {
                Pattern pat = aln->getPattern(i);
                s += aln->convertStateBack(pat[node->id]);
            }
            // for (auto m : node_branch->mutations) cout << m.get_string() << "+" << pos[m.position] << " ";
            return s;
        }
        else
        {
            ...
            if ((*it)->node->name != ROOT_NAME)
            {
                // ((PhyloNeighbor*)(*it))->distance = dad_branch->distance + 1;
                if (cur == 0)
                    left_branch = (PhyloNeighbor *)(*it)->node->findNeighbor(node), left = checkPartialMutation(pos, (PhyloNeighbor *)(*it), (PhyloNode *)node), cur = 1;
                else
                    right_branch = (PhyloNeighbor *)(*it)->node->findNeighbor(node), right = checkPartialMutation(pos, (PhyloNeighbor *)(*it), (PhyloNode *)node);
            }
            for (auto m : left_branch->mutations)
            {
                assert(pos[m.position] < (int)left.length());
                left[pos[m.position]] = aln->getStateFromMutation(m.par_nuc);
            }
            for (auto m : right_branch->mutations)
            {
                assert(pos[m.position] < (int)right.length());
                right[pos[m.position]] = aln->getStateFromMutation(m.par_nuc);
            }
            ...
            if (left != right)
            {
                ...
                {
                    if (left[i] != right[i] && (aln->getMutationFromState(left[i]) & aln->getMutationFromState(right[i])) == 0)
                    {
...
void PhyloTree::checkMutationBranch(vector<int> &pos, PhyloNeighbor *dad_branch, PhyloNode *dad, int *branch_subst)
{
    PhyloNode *node = (PhyloNode *)dad_branch->node;
    PhyloNeighbor *node_branch = (PhyloNeighbor *)node->findNeighbor(dad);
    assert(node_branch);
    ...
    string s = checkPartialMutation(pos, dad_branch, dad);
    string t = checkPartialMutation(pos, node_branch, node);
    for (auto m : node_branch->mutations)
    {
        s[pos[m.position]] = aln->getStateFromMutation(m.par_nuc);
    }
    for (auto m : dad_branch->mutations)
    {
        t[pos[m.position]] = aln->getStateFromMutation(m.par_nuc);
    }
    if (s != t)
    {
        for (int i = 0; i < (int)s.length(); ++i)
        {
            if (s[i] != t[i] && (aln->getMutationFromState(s[i]) & aln->getMutationFromState(t[i])) == 0)
            {
...
void PhyloTree::checkMutation(vector<int> &pos)
{
    ...
    checkMutationBranch(pos, (PhyloNeighbor *)root->neighbors[0], (PhyloNode *)root);
    cout << "========== End checking mutations ==========\n";
}

PhyloNode *PhyloTree::findNode(PhyloNode *node, PhyloNode *dad, string name)
{
    if (node->name == name)
    {
        return node;
    ...
    PhyloNode *found = NULL;
    FOR_NEIGHBOR_IT(node, dad, it)
    {
        found = findNode((PhyloNode *)(*it)->node, node, name);
        if (found)
        {
            return found;
        }
...
PhyloNode *PhyloTree::findNode(string name)
{
    PhyloNeighbor *nei = ((PhyloNeighbor *)root->neighbors[0]);
    return findNode((PhyloNode *)nei->node, (PhyloNode *)root, name);
}

bool PhyloTree::checkBranch(PhyloNode *node1, PhyloNode *node2)
{
    if (node2->dad->name == node1->name)
    {
        swap(node1, node2);
    }
    FOR_NEIGHBOR_DAD(node1, node2, it)
    {
        if (((PhyloNeighbor *)(*it))->check_can_do_spr())
        {
            return true;
        }
    }
...
int PhyloTree::matOptimizeSubtreeRegrafting(int cur_score, int cur_depth, PhyloNode *node1, PhyloNode *dad1, PhyloNode *orig_node1,
                                            PhyloNode *orig_node2, PhyloNode *node2, PhyloNode *dad2)
{
    if (node2->name == root->name || dad2->name == root->name)
    {
        return cur_score;
    }
    // cout << "start matOptimizeSubtreeRegrafting\n";
    ...
    if (dad2 && cur_depth >= SPR_DEPTH && checkBranch(node1, dad1) && checkBranch(node2, dad2))
    {
        // Connect branch (node1, dad1) to branch (node2, dad2).
        bool first = true;
        ...
        FOR_NEIGHBOR_IT(dad1, node1, it)
        {
            ...
            {
                (*it)->node = dad2;
                dad2->updateNeighbor(node2, dad1, -1.0);
                first = false;
            }
            else
            {
                (*it)->node = node2;
                node2->updateNeighbor(dad2, dad1, -1.0);
            }
        }
        ...
        // Swap back
        node2->updateNeighbor(dad1, dad2, -1.0);
        dad2->updateNeighbor(dad1, node2, -1.0);
    }
}

int PhyloTree::tryMatOptimize(int old_score, bool canRemove, PhyloNode *node, PhyloNode *dad, int sprDist)
{
    PhyloNeighbor *dad1_nei = NULL;
    PhyloNeighbor *dad2_nei = NULL;
    PhyloNode *sibling1 = NULL;
    ...
    // Find new dad for adding sibling.
    // sibling1: dad's par, sibling2: dad's child != node.
    if (dad && !dad->isLeaf())
    {
        assert(dad->degree() == 3);
        ...
        if (dad->dad == sibling2)
        {
            swap(dad1_nei, dad2_nei);
            swap(sibling1, sibling2);
        }

        // Save mutations.
        PhyloNeighbor *sibling1_branch = (PhyloNeighbor *)dad->findNeighbor(sibling1);
        PhyloNeighbor *sibling2_branch = (PhyloNeighbor *)sibling2->findNeighbor(dad);
        saveMutationBranch(node, dad);
        sibling1_branch->save_mutations();
        sibling2_branch->save_mutations();

        // Adding sibling to new dad.
        sibling1->updateNeighbor(dad, sibling2, -1.0);
        sibling2->updateNeighbor(dad, sibling1, -1.0);
        PhyloNeighbor *sibling1_nei = (PhyloNeighbor *)sibling1->findNeighbor(sibling2);
        PhyloNeighbor *sibling2_nei = (PhyloNeighbor *)sibling2->findNeighbor(sibling1);
        ...
        FOR_NEIGHBOR(sibling1, sibling2, it)
        {
            int score = matOptimizeSubtreeRegrafting(old_score, 1, node, dad, sibling1, sibling2, (PhyloNode *)(*it)->node, sibling1);
            if (score < old_score)
            {
                return score;
            }
        ...
        FOR_NEIGHBOR(sibling2, sibling1, it)
        {
            int score = matOptimizeSubtreeRegrafting(old_score, 1, node, dad, sibling2, sibling1, (PhyloNode *)(*it)->node, sibling2);
            if (score < old_score)
            {
                return score;
            }
        ...
        // Swap back.
        sibling1->updateNeighbor(sibling2, dad, -1.0);
        sibling2->updateNeighbor(sibling1, dad, -1.0);
        dad1_nei->node = sibling1;
        // dad1_nei->length = -1.0; : likelihood only.
        dad2_nei->node = sibling2;
        // dad2_nei->length = -1.0; : likelihood only.
    }

    FOR_NEIGHBOR_IT(node, dad, it)
    {
        PhyloNeighbor *it_back = (PhyloNeighbor *)(*it)->node->findNeighbor(node);
        double new_score = tryMatOptimize(old_score, it_back->check_can_do_spr(), (PhyloNode *)(*it)->node, node, sprDist);
...
void PhyloTree::tryMatOptimize(int sprDist)
{
    int oldScore = computeParsimonyScoreMutation();
    PhyloNeighbor *nei = (PhyloNeighbor *)root->neighbors[0];
    tryMatOptimize(oldScore, false, (PhyloNode *)nei->node, (PhyloNode *)root, sprDist);
}

void PhyloTree::initCandidate(vector<PhyloNode *> src, int sprDist)
{
    queue<pair<PhyloNeighbor *, int>> q;

    for (auto node : src)
    {
        for (auto tmp : node->neighbors)
        {
            PhyloNeighbor *nei = (PhyloNeighbor *)tmp;
            if (nei->distance != 0)
            {
                nei->distance = 0;
                PhyloNeighbor *nei_back = (PhyloNeighbor *)nei->node->findNeighbor(node);
                nei_back->distance = 0;
                q.push(make_pair(nei, 0));
                q.push(make_pair(nei_back, 0));
            }
        }
    }
    ...
    while (q.size())
    {
        PhyloNeighbor *nei = q.front().first;
        ...
        {
            PhyloNeighbor *nei = (PhyloNeighbor *)tmp;
            if (nei->distance > dist + 1)
            {
                nei->distance = dist + 1;
                PhyloNeighbor *nei_back = (PhyloNeighbor *)nei->node->findNeighbor(node);
                nei_back->distance = dist + 1;
                q.push(make_pair(nei, dist + 1));
                q.push(make_pair(nei_back, dist + 1));
            }
        }
    }
}

void PhyloTree::saveMutationBranch(PhyloNode *node, PhyloNode *dad)
{
    PhyloNeighbor *node_branch = (PhyloNeighbor *)node->findNeighbor(dad);
    node_branch->save_mutations();
    FOR_NEIGHBOR_IT(node, dad, it)
    {
        saveMutationBranch((PhyloNode *)(*it)->node, node);
    }
}